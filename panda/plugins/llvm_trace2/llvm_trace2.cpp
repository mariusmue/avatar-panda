...
const char *infer_register(uint32_t offset) { return ""; }
#endif
#endif

// write record to pandalog
void recordLoad(uint64_t address) {
    ...
    if (pandalog) {
        ...
        uint64_t archstate = (uint64_t)cpus.tqh_first->env_ptr;

        // printf("Address: %lx, firstcpu: %lx - %lx\n", address,
        // (uint64_t)first_cpu, (uint64_t)first_cpu + sizeof(CPUState));
        // printf("Address: %lx, cpuarchstate: %lx - %lx\n", address, archstate,
        // archstate + sizeof(CPUState));
        // printf("archstate: %x, %lx\n", archstate, (uint64_t)archstate);
        // printf("sizeof(CPUState): %x, sizeof(CPUArchState): %x,
        // sizeof(CPUX86State) :%x\n", sizeof(CPUState), sizeof(CPUArchState),
        // sizeof(CPUX86State));
        // sleep(10);

        // try to understand the source of the load
        if ((address >= archstate) &&
            (address < archstate + sizeof(CPUState))) {
            uint32_t reg_offset = (address - archstate) / 4;
            /*printf("%u\n", reg_offset);*/
            // printf("%s\n", infer_register(reg_offset));

            // set the address type as guest register, or a field in the CPU
            // state, because the address is contained between the start and the
            // end of the CPU state
            ple->mutable_llvmentry()->set_addr_type(TGT);  // Something in CPU
                                                           // state, may not
                                                           // necessarily be a
                                                           // register
            ple->mutable_llvmentry()->set_cpustate_offset(
                reg_offset);  // Something in CPU state, may not necessarily be
                              // a register
            // TODO: Fix this and store
        } else {
            // otherwise label it as memory
...
// same procedure as for recordLoad()
...
void write_trace_log() { printf("writing trace log\n"); }

// this function is inlined into LLVM assembly, writes dynamic values to trace
// log in protobuf format
void log_dynval() { write_trace_log(); }

extern "C" {
extern TCGLLVMContext *tcg_llvm_ctx;
}

// called by init_plugin()
...
void instrumentBasicBlock(BasicBlock &BB) {
    ...
    if (BB.getParent()->getName().startswith("tcg-llvm-tb-")) {
        int tb_num;
        sscanf(BB.getParent()->getName().str().c_str(), "tcg-llvm-tb-%d-%*d",
               &tb_num);
...
static RegisterPass<PandaLLVMTracePass> Y(
    "PandaLLVMTrace", "Instrument instructions that produce dynamic values");

// it runs over each basic block, as a consequence of the basic block pass
...
bool PandaLLVMTracePass::doInitialization(Module &module) {
    ...
    PLTV->recordLoadF = cast<Function>(module.getOrInsertFunction(
        "recordLoad", VoidType, VoidPtrType, nullptr));
    PLTV->recordStoreF = cast<Function>(module.getOrInsertFunction(
        "recordStore", VoidType, VoidPtrType, Int64Type, nullptr));
    PLTV->recordCallF = cast<Function>(
        module.getOrInsertFunction("recordCall", VoidType, Int64Type, nullptr));
    PLTV->recordSelectF = cast<Function>(module.getOrInsertFunction(
        "recordSelect", VoidType, Int8Type, nullptr));
    PLTV->recordSwitchF = cast<Function>(module.getOrInsertFunction(
        "recordSwitch", VoidType, Int64Type, nullptr));
    PLTV->recordBranchF = cast<Function>(module.getOrInsertFunction(
        "recordBranch", VoidType, Int8Type, nullptr));
    // recordStartBB:
    PLTV->recordStartBBF = cast<Function>(module.getOrInsertFunction(
        "recordStartBB", VoidType, VoidPtrType, Int64Type, nullptr));
    PLTV->recordBBF = cast<Function>(module.getOrInsertFunction(
        "recordBB", VoidType, VoidPtrType, Int32Type, nullptr));
    PLTV->recordReturnF = cast<Function>(
        module.getOrInsertFunction("recordReturn", VoidType, nullptr));
...
void PandaLLVMTraceVisitor::visitStoreInst(StoreInst &I) {
    ...
    Value *address = castTo(I.getPointerOperand(), VoidPtrType,
                            I.getPointerOperand()->getName(), &I);
 ...
 * llvm.memset is recorded as a Store, and llvm.memcpy is recorded as first a
 * Load from src and second a Store to dest
 */
...
void PandaLLVMTraceVisitor::handleExternalHelperCall(CallInst &I) {
    ...
    if (Regex("helper_[lb]e_ld.*_mmu_panda").match(name)) {
        ...
        dest = CastInst::Create(Instruction::IntToPtr, dest, VoidPtrType,
                                dest->getName(), &I);
        ...
        // storeVal = CastInst::Create(Instruction::IntToPtr, storeVal,
        // VoidPtrType, storeVal->getName(), &I);
...
void PandaLLVMTraceVisitor::visitCallInst(CallInst &I) {
    ...
    if (calledFunc->isIntrinsic()) {
        ...
        // model the MMU load/store functions with regular loads/stores from
        // dmemory
...
int before_block_exec(CPUState *env, TranslationBlock *tb) {
    // printf("first_cpu->env_ptr: %lx\n", (uint64_t)first_cpu->env_ptr);
    // printf("(uint64_t)cpus.tqh_first->env_ptr: %lx\n",
    //(uint64_t)cpus.tqh_first->env_ptr);
    // printf("first_cpu: %lx\n", (uint64_t)first_cpu);
    // printf("(uint64_t)cpus.tqh_first: %lx\n", (uint64_t)cpus.tqh_first);
    // printf("a == b = %d\n", (uint64_t)first_cpu == (uint64_t)cpus.tqh_first);
    ...
    if (pandalog) {
        ...
        if (tb->llvm_function->getName().startswith("tcg-llvm-tb-")) {
            int tb_num;
            sscanf(tb->llvm_function->getName().str().c_str(),
                   "tcg-llvm-tb-%d-%*d", &tb_num);
...
bool init_plugin(void *self) {
    ...
    for (llvm::Module::iterator func = module->begin(), mod_end = module->end();
         func != mod_end; ++func) {
        for (llvm::Function::iterator b = func->begin(), be = func->end();
             b != be; ++b) {
...
void uninit_plugin(void *self) {
    ...
    llvm::Constant *cpustate = llvm::ConstantInt::get(
        llvm::Int64Type, (uint64_t)cpus.tqh_first->env_ptr);
    // Add a global variable of the CPUState address
    llvm::GlobalVariable *CPUStateAddr = new llvm::GlobalVariable(
        /*Module=*/*mod,
